...
namespace Taas {
    ...
    public:
        ...
        static bool SendTxnToServer(const Context& ctx, uint64_t& epoch, uint64_t& to_whom, std::shared_ptr<proto::Transaction> txn_ptr, proto::TxnType txn_type);
        static bool SendRemoteServerTxn(const Context& ctx, uint64_t& epoch, uint64_t& to_whom, std::shared_ptr<proto::Transaction> txn_ptr, proto::TxnType txn_type);
        static bool SendBackUpTxn(const Context &ctx, uint64_t& epoch, std::shared_ptr<proto::Transaction> txn_ptr, proto::TxnType txn_type);
