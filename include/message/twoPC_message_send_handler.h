...
namespace Taas {
    ...
    public:
        ...
        static bool SendTxnCommitResultToClient(const Context& ctx, const std::shared_ptr<proto::Transaction>& txn_ptr, proto::TxnState txn_state);
        static bool SendTxnToServer(const Context& ctx, uint64_t& to_whom, const std::shared_ptr<proto::Transaction>& txn_ptr, proto::TxnType txn_type);
        static bool SendRemoteServerTxn(const Context& ctx, uint64_t& to_whom, const std::shared_ptr<proto::Transaction>& txn_ptr, proto::TxnType txn_type);
