...
namespace Taas {
    ...
    std::unique_ptr<moodycamel::BlockingConcurrentQueue<std::unique_ptr<zmq::message_t>>>
...
//    std::unique_ptr<BlockingConcurrentQueue<std::unique_ptr<proto::Transaction>>> MessageQueue::listen_message_txn_queue, MessageQueue::listen_message_epoch_queue;
    ...
    void MessageQueue::StaticInitMessageQueue(const Context& ctx) {
        ...
        request_queue = std::make_unique<moodycamel::BlockingConcurrentQueue<std::unique_ptr<proto::Message>>>();
        raft_message_queue = std::make_unique<moodycamel::BlockingConcurrentQueue<std::unique_ptr<proto::Message>>>();
    }
