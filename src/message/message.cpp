...
namespace Taas {
    ...
    std::unique_ptr<BlockingConcurrentQueue<std::unique_ptr<zmq::message_t>>>
...
//    std::unique_ptr<BlockingConcurrentQueue<std::unique_ptr<proto::Transaction>>> MessageQueue::listen_message_txn_queue, MessageQueue::listen_message_epoch_queue;
    std::unique_ptr<BlockingConcurrentQueue<std::unique_ptr<send_params>>> MessageQueue::send_to_server_queue,
    ...
    void MessageQueue::StaticInitMessageQueue(const Context& ctx) {
        ...
        listen_message_txn_queue = std::make_unique<BlockingConcurrentQueue<std::unique_ptr<zmq::message_t>>>();
        listen_message_epoch_queue = std::make_unique<BlockingConcurrentQueue<std::unique_ptr<zmq::message_t>>>();
