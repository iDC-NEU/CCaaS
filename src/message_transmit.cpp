 ...
 * 5555 :
 * 5556 : txn nodes sends log to storage nodes                                   txn PUB            storage SUB
 *
 * 5557 :
 * 5558 : txn nodes sends txns to other txn nodes; txn nodes send raft states    txn PUB            other txn SUB
 * @param data
 * @param hint
 */
...
//    void SendStoragePUBThreadMain2(uint64_t id, Context ctx); //PUB pack txn
//
//    void SendServerThreadMain(uint64_t id, Context ctx);//PUB
//    void ListenServerThreadMain(uint64_t id, Context ctx);//SUB
...
//    socket_send.bind("tcp://*:5557");//to server
        socket_send.bind("tcp://*:5558");//to server
        int queue_length = 0;
        socket_send.setsockopt(ZMQ_SNDHWM, &queue_length, sizeof(queue_length));
 ...
 */
    void ListenServerThreadMain(uint64_t id, Context ctx) {///监听远端txn node写集
        ...
        for (int i = 0; i < (int) ctx.kServerIp.size(); i++) {
            ...
            socket_listen.connect("tcp://" + ctx.kServerIp[i] + ":5557");
            socket_listen.connect("tcp://" + ctx.kServerIp[i] + ":5558");
